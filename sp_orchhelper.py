...
########## applicationDefinition ##########
    ...
    def delete_userDefined_app_addressMap(self, ipStart, ipEnd):
        # DELETE operation to delete specific user defined application group based on address map
        # IP addresses must be passed as 32-bit integer values (e.g. 167837953 rather than 10.1.1.1)

        #Convert ip address strings into IPv4 Decimal format for error message
        ipStartDec = ipaddress.ip_address(ipStart)
        ipEndDec = ipaddress.ip_address(ipEnd)

        response = self.delete("/applicationDefinition/ipIntelligenceClassification/{0}/{1}".format(ipStart,ipEnd))
        if response.status_code == 200:
            return True
        else:
            print (response.status_code)
            print("Failed to delete application address map (Range {0} - {1}) on Orch at {2}".format(ipStartDec, ipEndDec, self.url))
            return False
    
    def post_userDefined_app_addressMap(self, ipStart, ipEnd, name="", description="", priority="", country="", country_code="", org=""):
        # POST operation to create or update user defined application groups based on address map
        # IP addresses must be passed as 32-bit integer values (e.g. 167837953 rather than 10.1.1.1)
        ...
        ipIntelligenceConfig = {"ip_start":ipStart, "ip_end":ipEnd, "name":name, "description":description, "priority":priority, "country":country, "country_code":country_code, "org":org}

        response = self.post("/applicationDefinition/ipIntelligenceClassification/{0}/{1}".format(ipStart, ipEnd), ipIntelligenceConfig)
        if response.status_code == 200:
            print("Address Map {0} added/updated successfully".format(name))
            return response
        else:
            print (response.status_code)
            print("Failed to create or update application address map {0} on Orch at {1}".format(name, self.url))
            return False


    def get_userDefined_appGroups(self):
        # GET operation to retrieve user defined application groups
        response = self.get("/applicationDefinition/applicationTags?resourceKey=userDefined")
        if response.status_code == 200:
            return response
        else:
            print("Failed to retrieve user defined applications from Orch at {0}".format(self.url))
            return False

    def update_userDefined_appGroups(self, appTagsBody):
        # POST operation to repost user defined application groups
        response = self.post("/applicationDefinition/applicationTags", appTagsBody)
        if response.status_code == 200:
            if appTagsBody == {}:
                print("Blanked User Defined Application Groups")
                return response
            else:
                print("User Defined Application Groups updated successfully")
                return response
        else:
            print (response.status_code)
            print("Failed to update User Defined Application Groups on Orch at {1}".format(self.url))
            return False
